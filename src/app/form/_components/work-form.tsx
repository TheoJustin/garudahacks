...
export default function WorkForm({
  ...
  const next = async (direction : 1 | -1) => {
    ...
    appendFormData({
      workExperience: hasWorkExperience ? workYears : 0
    })
    ...
    >
      ...
      <div className="w-full">
        ...
        <div className={`w-full px-16 overflow-hidden transition-all duration-200 ${!hasWorkExperience ? "h-0 mt-0" : "h-10 mt-8"}`}>
          <Input 
            className="border-slate-500" placeholder="Work experiences in years" type={"number"}
            value={workYears} onChange={(e) => setWorkYears(parseInt(e.target.value))}
          />
        </div>
      </div>
